-- ==========================================================
-- Versao segura para colar no Supabase SQL Editor.
-- Nao usa dollar-quoted strings ($$ ou $tag$), para evitar
-- o erro "unterminated dollar-quoted string" quando o painel
-- corta/seleciona parte do script.
-- ==========================================================
...
BEGIN
  ...
  IF v_key IS NULL THEN
    RAISE EXCEPTION ''Chave de criptografia nao configurada em app_config.'';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION ''Acesso negado: paciente nao pertence ao profissional autenticado.'';
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION ''Acesso negado: ficha nao pertence ao profissional autenticado.'';
  END IF;
